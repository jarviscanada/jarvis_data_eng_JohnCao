...
WITH mem_usage_to_nearest_interval (host_id, nearest_interval, mem_used) AS (
    SELECT host_id, (date_trunc('hour', host_usage.timestamp) + date_part('minute', host_usage.timestamp):: int / 5 * interval '5 min') as nearest_interval,
    ...
    SELECT host_id, nearest_interval, AVG(mem_used) as avg_used_memory
    FROM host_info as h, mem_usage_to_nearest_interval as m
    WHERE h.id = m.host_id
    GROUP BY host_id, nearest_interval
)

SELECT host_id, hostname, nearest_interval as "timestamp", ROUND(avg_used_memory::DECIMAL / total_mem * 100) as avg_used_mem_percentage
...
FROM (
         SELECT host_id, (date_trunc('hour', timestamp) + date_part('minute', timestamp):: int / 5 * interval '5 min') as "timestamp"
         FROM host_usage
    ) as entries
